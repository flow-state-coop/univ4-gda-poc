...
contract AqueductTest is Test {
    ...
    function test_1lp_bootstrap(uint64 r1, uint64 r2, uint16 dt1) external {
        ...
        emit log_named_int(
            "token1 r a->x", int256(FlowRate.unwrap(token1.getFlowRate(alice, address(x), FlowId.wrap(0))))
        );
        emit log_named_int(
            "token2 r a->x", int256(FlowRate.unwrap(token2.getFlowRate(alice, address(x), FlowId.wrap(0))))
        );
        emit log_named_int(
            "token1 r x->a", int256(FlowRate.unwrap(token1.getFlowRate(address(x), alice, FlowId.wrap(0))))
        );
        emit log_named_int(
            "token2 r x->a", int256(FlowRate.unwrap(token2.getFlowRate(address(x), alice, FlowId.wrap(0))))
        );
    ...
    function test_1lp_1trader(uint64 r1, uint64 r2, uint16 dt1, uint64 r3, uint16 dt2) external {
        ...
        emit log_named_int(
            "token1 r b->x", int256(FlowRate.unwrap(token1.getFlowRate(bob, address(x), FlowId.wrap(0))))
        );
        emit log_named_int(
            "token1 r x->a", int256(FlowRate.unwrap(token1.getFlowRate(address(x), alice, FlowId.wrap(0))))
        );
        emit log_named_int(
            "token1 r x->b", int256(FlowRate.unwrap(token1.getFlowRate(address(x), bob, FlowId.wrap(0))))
        );
        ...
        emit log_named_int(
            "token2 r b->x", int256(FlowRate.unwrap(token2.getFlowRate(bob, address(x), FlowId.wrap(0))))
        );
        emit log_named_int(
            "token2 r x->a", int256(FlowRate.unwrap(token2.getFlowRate(address(x), alice, FlowId.wrap(0))))
        );
        emit log_named_int(
            "token2 r x->b", int256(FlowRate.unwrap(token2.getFlowRate(address(x), bob, FlowId.wrap(0))))
        );
        ...
        if (
            r1 > 0 && r2 > 0 && r3 > 0 && uint256(r2) >= uint256(r1 / 1e9 + 1) + uint256(r3 / 1e9 + 1) && dt1 > 0
                && dt2 > 0
        ) {
    ...
    function test_random_seqs(Step[20] memory steps) external {
        ...
        for (uint256 i = 0; i < steps.length; ++i) {
            ...
            uint256 u = 1 + s.u % 5; // a pool of 5 testers
