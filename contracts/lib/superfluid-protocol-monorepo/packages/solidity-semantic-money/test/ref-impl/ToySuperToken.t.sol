...
import {Value, FlowRate, Time, Unit} from "@superfluid-finance/solidity-semantic-money/src/SemanticMoney.sol";
...
contract ToySuperTokenTest is Test {
    ...
    uint256 internal constant INIT_BALANCE = 1e20;
    ...
    function test_1to2_flow(uint32 r1, uint32 r2, uint16 dt1) external {
        ...
        assertEq(-token.getNetFlowRate(alice), token.getNetFlowRate(bob) + token.getNetFlowRate(carol), "e2");
    ...
    function test_1to2_instdistribute(uint32 u1, uint32 u2, uint64 x) external {
        ...
        Value xx = Value.wrap(int256(uint256(x)));
        uint256 tu = uint256(uint128(Unit.unwrap(uu1 + uu2)));
        uint256 xxx;
        if (tu == 0) xxx = 0;
        else xxx = uint256(x) / tu * tu;
    ...
    function test_1to2_distributeflow_bothconnected(uint32 u1, uint32 u2, uint32 r1, uint16 dt1, uint32 r2, uint16 dt2)
        external
    {
        Unit uu1 = Unit.wrap(int128(uint128((u1))));
        Unit uu2 = Unit.wrap(int128(uint128((u2))));
        ...
        FlowRate rrr1;
        if (tu != 0) rrr1 = FlowRate.wrap(int128(uint128(r1) / tu * tu));
        FlowRate rrr2;
        if (tu != 0) rrr2 = FlowRate.wrap(int128(uint128(r2) / tu * tu));
    ...
    function test_1to2_distributeflow_oneconnected(uint32 u1, uint32 u2, uint32 r, uint16 dt2) external {
        ...
        uint128 tu = uint128(Unit.unwrap(uu1 + uu2));
        FlowRate rrr;
        if (tu != 0) rrr = FlowRate.wrap(int128(uint128(r) / tu * tu));
    ...
    function test_1to1_distributeflow_connect_disconnect_connect(
        uint32 u,
        uint32 r,
        uint16 dt1, // connect
    ...
    function test_1to2_distributeflow_unit_updates(uint32 u1, uint32 r1, uint16 dt1, uint32 u2, uint32 r2, uint16 dt2)
        external
    {
        Unit uu1 = Unit.wrap(int128(uint128((u1))));
        FlowRate rr1 = FlowRate.wrap(int64(uint64(r1)));
        ...
        int256 rrr1;
        if (tu == 0) rrr1 = 0;
        else rrr1 = int256(uint256(r1) / tu * tu);
        int256 rrr2;
        if (tu == 0) rrr2 = 0;
        else rrr2 = int256(uint256(r2) / tu * tu);
        Time t1 = Time.wrap(uint32(block.timestamp) + dt1);
        Time t2 = t1 + Time.wrap(dt2);
    ...
    function test_2to1_distributeflow(uint32 u1, uint32 r1, uint32 r2, uint16 dt2) external {
        ...
        assertEq(Unit.unwrap(pl.getDisconnectedUnits()), int256(tu), "e1");
    ...
    function test_pool_distributeflow_claim_connected_pool(
        uint32 u,
        uint32 r,
        uint16 dt1, // claimAll
        uint16 dt2
    ...
    function test_pool_distributeflow_update_unit(uint32 u1, uint32 r, uint16 dt1, uint32 u2, uint16 dt2) external {
        Unit uu1 = Unit.wrap(int128(uint128((u1))));
        Unit uu2 = Unit.wrap(int128(uint128((u2))));
        FlowRate rr = FlowRate.wrap(int128(uint128(r)));
        FlowRate rrr1;
        if (u1 != 0) rrr1 = rr.div(uu1).mul(uu1);
        FlowRate rrr2;
        if (u2 != 0) rrr2 = rrr1.div(uu2).mul(uu2);
        Time t0 = Time.wrap(uint32(block.timestamp));
        Time t1 = t0 + Time.wrap(uint32(dt1));
        Time t2 = t1 + Time.wrap(uint32(dt2));
    ...
    function test_pool_random_seqs(PoolUpdateStep[20] memory steps) external {
        ...
        for (uint256 i = 0; i < steps.length; ++i) {
            ...
            uint256 a = s.a % 4;
            uint256 u = 1 + s.u % nMembers; // a pool of 5 testers including the pool creator
            ...
            if (a == 0) {
                ...
                (, FlowRate ar, FlowRate pdr) =
                    token.distributeFlow(TEST_ACCOUNTS[u], pl, FlowId.wrap(0), FlowRate.wrap(int128(uint128(s.v))));
                emit log_named_int("ar", FlowRate.unwrap(ar));
                emit log_named_int("pdr", FlowRate.unwrap(pdr));
                vm.stopPrank();
            } else if (a == 2) {
                uint256 u4 = 1 + s.v % nMembers;
