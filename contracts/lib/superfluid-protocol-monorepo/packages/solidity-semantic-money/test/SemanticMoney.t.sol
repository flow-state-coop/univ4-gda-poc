...
contract SemanticMoneyTest is Test {
    ...
    function test_flowrate_mul_unit_distributive_law(int64 r_, int64 u1_, int64 u2_) external pure {
        ...
        if (
            int256(r_) * (int256(u1_) + int256(u2_)) > type(int128).max
                || int256(r_) * (int256(u1_) + int256(u2_)) < type(int128).min
        ) return;
    ...
    function uu_shift2(BasicParticle memory a, BasicParticle memory b, Time, /* t */ int64 v)
        internal
        pure
        returns (BasicParticle memory, BasicParticle memory)
    {
    ...
    function run_uu_test(
        ...
        function (BasicParticle memory, BasicParticle memory, Time, int64)
                         internal pure returns (BasicParticle memory, BasicParticle memory) op2
    ) internal pure {
        UUTestVars memory d;
        d.t1 = Time.wrap(m1);
    ...
    function test_uu_flow2(uint16 m1, int64 r1, uint16 m2, int64 r2, uint16 m3) external pure {
        ...
        assertEq(Value.unwrap(d.b.rtb(d.t3)), int256(r1) * int256(uint256(m2)) + int256(r2) * int256(uint256(m3)));
    }
    ...
    function updp_shift2(BasicParticle memory a, PDPoolIndex memory b, Time, /* t */ int64 v)
        internal
        pure
        returns (BasicParticle memory a2, PDPoolIndex memory b2)
    {
    ...
    function test_updp_1m_ss(
        uint16 m1,
        int64 u1, // update unit
        uint16 m2,
        int64 x2, // distribute
        uint16 m3,
        int64 u2, // update unit
        uint16 m4,
        int64 x4, // distribute
        uint16 m5
    ) external pure {
    ...
    function test_updp_1m_ff(
        uint16 m1,
        int64 u1, // update unit
        uint16 m2,
        int64 r2, // distribute flow
        uint16 m3,
        int64 u2, // update unit
        uint16 m4,
        int64 r4, // distribute flow
        uint16 m5
    ) external pure {
        run_updp_1m_test(m1, u1, m2, r2, updp_flow2, m3, u2, m4, r4, updp_flow2, m5);
    }

    function test_updp_1m_sf(
        uint16 m1,
        int64 u1, // update unit
        uint16 m2,
        int64 x2, // distribute
    ...
    function run_updp_2m_test( // update unit
        ...
        assertEq(
            Value.unwrap(d.a.rtb(d.t5) + PDPoolMemberMU(d.b, d.b1).rtb(d.t5) + PDPoolMemberMU(d.b, d.b2).rtb(d.t5)),
            0,
            "e5"
        );
    }

    function test_updp_2m_ss(
        uint16 m1,
        int64 u1, // update unit
        uint16 m2,
        int64 x2, // distribute
    ...
    function test_updp_flow2(uint16 m1, int64 r1, uint16 m2, int64 r2, uint16 m3) external pure {
        ...
        assertEq(
            Value.unwrap(PDPoolMemberMU(d.b, d.b1).rtb(d.t3)),
