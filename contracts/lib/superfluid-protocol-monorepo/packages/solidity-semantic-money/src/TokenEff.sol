...
struct TokenEff {
    ...
    function (TokenEff memory, address /*owner*/) internal view returns (BasicParticle memory) getUIndex;
    function (TokenEff memory, address /*owner*/, BasicParticle memory) internal returns (TokenEff memory) setUIndex;
    function (TokenEff memory, address /*pool*/) internal view returns (PDPoolIndex memory) getPDPIndex;
    function (TokenEff memory, address /*pool*/, PDPoolIndex memory) internal returns (TokenEff memory) setPDPIndex;
    function (TokenEff memory, bytes32 /*flowHash*/) internal view returns (FlowRate) getFlowRate;
    function (TokenEff memory, bytes32 /*flowHash*/, address /*from*/, address /*to*/, FlowRate /*flowRate*/)
        internal returns (TokenEff memory) setFlowInfo;
    /*
...
library TokenEffLib {
    ...
    function shift(TokenEff memory eff, address from, address to, Value amount) internal returns (TokenEff memory) {
        ...
        return eff.setUIndex(from, eff.getUIndex(from).mappend(a)).setUIndex(to, eff.getUIndex(to).mappend(b));
    }
        ...
        returns (TokenEff memory)
    {
        ...
        return eff.setUIndex(from, a).setUIndex(to, b).setFlowInfo(eff, flowHash, from, to, flowRate);
    }

    function distribute(TokenEff memory eff, address from, address to, Value reqAmount)
        internal
        returns (TokenEff memory, Value actualAmount)
    {
        BasicParticle memory a = eff.getUIndex(from);
        PDPoolIndex memory pdpIndex = eff.getPDPIndex(to);
        (a, pdpIndex, actualAmount) = a.shift2b(pdpIndex, reqAmount);
        return (eff.setUIndex(from, a).setPDPIndex(to, pdpIndex), actualAmount);
    }
    ...
    function distributeFlow(
        ...
        return (
            eff.setUIndex(from, a).setPDPIndex(to, pdpIndex).setFlowInfo(flowHash, from, to, actualFlowRate),
            actualFlowRate
        );
    }
}
