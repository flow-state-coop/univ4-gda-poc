...
import {FlowRate, Unit} from "@superfluid-finance/solidity-semantic-money/src/SemanticMoney.sol";
import {
...
library AqueductLibrary {
    ...
    function updateSide(PoolSideState memory poolA, FlowRate r0, FlowRate r1)
        internal
        pure
...
contract Aqueduct {
    FlowId public constant SWAP_DISTRIBUTE_FLOW_ID = FlowId.wrap(0);
    FlowId public constant ADJUSTMENT_FLOW_ID = FlowId.wrap(0);
    ...
    function token1() external view returns (ToySuperfluidToken) {
        return _left.token;
    }

    function token2() external view returns (ToySuperfluidToken) {
        return _right.token;
    }

    function pool1() external view returns (ToySuperfluidPool) {
        return _left.pool;
    }

    function pool2() external view returns (ToySuperfluidPool) {
        return _right.pool;
    }
     ...
     */
    function _adjustFlowRemainder(Side storage a, address newRemFlowReceiver, FlowRate ird, FlowRate drd) internal {
        // * Given definitions:
        //
