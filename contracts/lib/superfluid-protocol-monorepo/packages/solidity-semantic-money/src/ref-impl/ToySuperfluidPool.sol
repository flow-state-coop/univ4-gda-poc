...
import {
    Time, Value, FlowRate, Unit, BasicParticle, PDPoolIndex, PDPoolMember, PDPoolMemberMU
} from "../SemanticMoney.sol";
import {ISuperfluidPool, ISuperfluidPoolOperator} from "./ISuperfluidPool.sol";
...
contract ToySuperfluidPool is Initializable, ISuperfluidPool {
    ...
    function getIndex() external view override returns (PDPoolIndex memory) {
        return _pdpIndex;
    }

    function getTotalUnits() external view override returns (Unit) {
        return _pdpIndex.total_units;
    }

    function getDisconnectedUnits() external view override returns (Unit) {
        return _disconnectedMembers.owned_units;
    }

    function getUnits(address memberAddr) external view override returns (Unit) {
    ...
    function getConnectedFlowRate() external view override returns (FlowRate) {
        return _pdpIndex.flow_rate_per_unit().mul(_pdpIndex.total_units);
    }

    function getDisconnectedFlowRate() external view override returns (FlowRate) {
        return _pdpIndex.flow_rate_per_unit().mul(_disconnectedMembers.owned_units);
    }

    function getDisconnectedBalance(Time t) external view override returns (Value) {
        return PDPoolMemberMU(_pdpIndex, _disconnectedMembers).rtb(t) - _claimedByDisconnectedMembers;
    }

    function getMemberFlowRate(address memberAddr) external view override returns (FlowRate) {
    ...
    function getClaimable(address memberAddr, Time t) public view override returns (Value) {
        return PDPoolMemberMU(_pdpIndex, _members[memberAddr]).rtb(t) - _claimedValues[memberAddr];
    }

    function getClaimable(address memberAddr) external view override returns (Value v) {
        Time t = Time.wrap(uint32(block.timestamp));
        v = getClaimable(memberAddr, t);
    }

    function updateMember(address memberAddr, Unit newUnits) external override returns (bool) {
    ...
    function claimAll(address memberAddr) public override returns (bool) {
        bool isConnected = ISuperfluidPoolOperator(POOL_OPERATOR).isMemberConnected(this, memberAddr);
        Time t = Time.wrap(uint32(block.timestamp));
        Value claimedAmount = _claimAll(memberAddr, t);
        if (!isConnected) {
    ...
    function operatorSetIndex(PDPoolIndex calldata index) external override onlyOperator returns (bool) {
        _pdpIndex = index;
        return true;
    }

    function operatorConnectMember(address memberAddr, bool doConnect, Time t)
        external
        override
        onlyOperator
        returns (bool)
