...
import {Time, Value, FlowRate, BasicParticle, PDPoolIndex} from "../SemanticMoney.sol";
import {TokenMonad} from "../TokenMonad.sol";
import {ISuperfluidPool, ToySuperfluidPool} from "./ToySuperfluidPool.sol";
...
contract ToySuperfluidToken is ISuperfluidToken, TokenMonad {
    ...
    function realtimeBalanceNow(address account) external view override returns (Value avb) {
        Time t = Time.wrap(uint32(block.timestamp));
        return realtimeBalanceAt(account, t);
    }

    function realtimeBalanceAt(address account, Time t) public view override returns (Value avb) {
        (Value own, Value fromPools,) = realtimeBalanceVectorAt(account, t);
        avb = own + fromPools;
    }

    function realtimeBalanceVectorNow(address account)
        public
        view
        override
    ...
    function realtimeBalanceVectorAt(address account, Time t)
        public
        view
        override
        returns (Value own, Value fromPools, Value deposit)
    ...
    function getNetFlowRate(address account) external view override returns (FlowRate nr) {
    ...
    function getFlowRate(address from, address to, FlowId flowId) external view override returns (FlowRate) {
    ...
    function _shift(bytes memory eff, address operator, address from, address to, Value amount)
        internal
    ...
    function shift(address from, address to, Value amount) external virtual override returns (bool success) {
    ...
    function _distributeFlow(
        ...
        // Make updates
        (eff, actualFlowRate, newDistributionFlowRate) =
            _doDistributeFlowViaPool(eff, from, address(to), flowHash, reqFlowRate, t);
    ...
    function createPool() external returns (ToySuperfluidPool pool) {
    ...
    function isPool(address p) external view override returns (bool) {
        return _isPool(p);
    }

    function getNumConnections(address account) external view override returns (uint256) {
        return _poolConnectionsMap[account].length();
    }

    function connectPool(ISuperfluidPool to) external override returns (bool success) {
        return connectPool(to, true);
    }

    function disconnectPool(ISuperfluidPool to) external override returns (bool success) {
        return connectPool(to, false);
    }

    function connectPool(ISuperfluidPool to, bool doConnect) public override returns (bool success) {
    ...
    function isMemberConnected(ISuperfluidPool to, address memberAddr) external view override returns (bool) {
    ...
    function appendIndexUpdateByPool(BasicParticle memory p, Time t) external virtual override returns (bool) {
    ...
    /// Settle the claim
    function poolSettleClaim(address claimRecipient, Value amount) external virtual override returns (bool) {
    ...
    function _getFlowRate(bytes memory, /*eff*/ bytes32 flowHash) internal view virtual override returns (FlowRate) {
    ...
    function _getPoolAdjustmentFlowInfo(bytes memory eff, address pool)
        internal
        view
        returns (address adjustmentRecipient, bytes32 flowHash, FlowRate)
    {
