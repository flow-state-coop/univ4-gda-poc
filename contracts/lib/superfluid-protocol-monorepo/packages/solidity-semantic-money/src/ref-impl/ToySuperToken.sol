...
contract ToySuperToken is ToySuperfluidToken, IERC20 {
    ...
    function _acl(address operator, address from, address, /* to */ Value shiftAmount, FlowRate shiftFlowRate)
        internal
        override
        returns (bool)
    {
    ...
    function shift(address from, address to, Value amount) external override returns (bool success) {
    ...
    function appendIndexUpdateByPool(BasicParticle memory p, Time t) external override returns (bool) {
        bytes memory eff = _tokenEff("appendIndexUpdateByPool", new bytes(0));
        _appendIndexUpdateByPool(eff, msg.sender, p, t);
        return true;
    }

    function poolSettleClaim(address claimRecipient, Value amount) external override returns (bool) {
    ...
    event UIndexSet(
        address indexed msgSender, string indexed prim, address indexed owner, BasicParticle p, bytes primEtra
    );
    ...
    event PDPIndexSet(
        address indexed msgSender, string indexed prim, address indexed pool, PDPoolIndex p, bytes primEtra
    );
    ...
    event FlowInfoSet(
        address indexed msgSender, string indexed prim, bytes32 indexed flowHash, FlowRate flowRate, bytes primEtra
    );
