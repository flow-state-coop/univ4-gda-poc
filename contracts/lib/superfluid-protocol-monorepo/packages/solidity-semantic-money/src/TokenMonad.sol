...
abstract contract TokenMonad {
    function _getUIndex(bytes memory eff, address owner) internal view virtual returns (BasicParticle memory);
    function _setUIndex(bytes memory eff, address owner, BasicParticle memory p)
        internal
        virtual
        returns (bytes memory);
    function _getPDPIndex(bytes memory eff, address pool) internal view virtual returns (PDPoolIndex memory);
    function _setPDPIndex(bytes memory eff, address pool, PDPoolIndex memory p)
        internal
        virtual
        returns (bytes memory);
    function _getFlowRate(bytes memory, bytes32 flowHash) internal view virtual returns (FlowRate);
    ...
    function _getPoolAdjustmentFlowRate(bytes memory eff, address pool) internal view virtual returns (FlowRate);
    ...
    function _doDistributeFlowViaPool(
        bytes memory eff,
        address from,
        address pool,
        bytes32 flowHash,
...

        {
            ...
            newActualFlowRate =
                oldFlowRate + (newDistributionFlowRate - oldDistributionFlowRate) - vars.currentAdjustmentFlowRate;
