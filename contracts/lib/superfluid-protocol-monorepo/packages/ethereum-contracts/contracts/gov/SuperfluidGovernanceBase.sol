...
abstract contract SuperfluidGovernanceBase is ISuperfluidGovernance {
     ...
     * /************************************************************************
     */
    function replaceGovernance(ISuperfluid host, address newGov) external override onlyAuthorized(host) {
        host.replaceGovernance(ISuperfluidGovernance(newGov));
    }

    function registerAgreementClass(ISuperfluid host, address agreementClass) external override onlyAuthorized(host) {
    ...
    event ConfigChanged(
        ISuperfluid indexed host, ISuperfluidToken indexed superToken, bytes32 key, bool isKeySet, uint256 value
    );

    function setConfig(ISuperfluid host, ISuperfluidToken superToken, bytes32 key, address value) external override {
        _setConfig(host, superToken, key, value);
    }

    function setConfig(ISuperfluid host, ISuperfluidToken superToken, bytes32 key, uint256 value) external override {
        _setConfig(host, superToken, key, value);
    }

    function clearConfig(ISuperfluid host, ISuperfluidToken superToken, bytes32 key) external override {
        _clearConfig(host, superToken, key);
    }

    function _setConfig(ISuperfluid host, ISuperfluidToken superToken, bytes32 key, address value)
        internal
        onlyAuthorized(host)
    {
    ...
    function _setConfig(ISuperfluid host, ISuperfluidToken superToken, bytes32 key, uint256 value)
        internal
        onlyAuthorized(host)
    {
    ...
    event RewardAddressChanged(
        ISuperfluid indexed host, ISuperfluidToken indexed superToken, bool isKeySet, address rewardAddress
    );
    ...
    event CFAv1LiquidationPeriodChanged(
        ISuperfluid indexed host, ISuperfluidToken indexed superToken, bool isKeySet, uint256 liquidationPeriod
    );
    ...
    function setPPPConfig(
        ...
        if (
            liquidationPeriod <= patricianPeriod || liquidationPeriod >= type(uint32).max
    ...
    event SuperTokenMinimumDepositChanged(
        ISuperfluid indexed host, ISuperfluidToken indexed superToken, bool isKeySet, uint256 minimumDeposit
    );
    ...
    event TrustedForwarderChanged(
        ISuperfluid indexed host, ISuperfluidToken indexed superToken, bool isKeySet, address forwarder, bool enabled
    );

    function isTrustedForwarder(ISuperfluid host, ISuperfluidToken superToken, address forwarder)
        external
        view
        returns (bool)
    {
        return getConfigAsUint256(host, superToken, SuperfluidGovernanceConfigs.getTrustedForwarderConfigKey(forwarder))
            == 1;
    }
    ...
    event AppRegistrationKeyChanged(
        ISuperfluid indexed host, address indexed deployer, string appRegistrationKey, uint256 expirationTs
    );

    function verifyAppRegistrationKey(ISuperfluid host, address deployer, string memory registrationKey)
        external
        view
    ...
    function isAuthorizedAppFactory(ISuperfluid host, address factory) external view returns (bool) {
        return getConfigAsUint256(
            host, ISuperfluidToken(address(0)), SuperfluidGovernanceConfigs.getAppFactoryConfigKey(factory)
        ) == 1;
    }
