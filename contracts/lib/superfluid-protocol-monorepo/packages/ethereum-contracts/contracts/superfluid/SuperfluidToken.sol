...
abstract contract SuperfluidToken is ISuperfluidToken {
        ...
        returns (int256 availableBalance, uint256 deposit, uint256 owedDeposit)
    {
        ...
        for (uint256 i = 0; i < activeAgreements.length; ++i) {
            (int256 agreementDynamicBalance, uint256 agreementDeposit, uint256 agreementOwedDeposit) =
            ...
            availableBalance = availableBalance + agreementDynamicBalance
    ...
    function isAccountCriticalNow(address account) external view virtual override returns (bool isCritical) {
    ...
    function isAccountSolventNow(address account) external view virtual override returns (bool isSolvent) {
    ...
    function createAgreement(bytes32 id, bytes32[] calldata data) external virtual override {
    ...
    function updateAgreementData(bytes32 id, bytes32[] calldata data) external virtual override {
        address agreementClass = msg.sender;
        bytes32 slot = keccak256(abi.encode("AgreementData", agreementClass, id));
        FixedSizeData.storeData(slot, data);
    ...
    function terminateAgreement(bytes32 id, uint256 dataLength) external virtual override {
        address agreementClass = msg.sender;
        bytes32 slot = keccak256(abi.encode("AgreementData", agreementClass, id));
    ...
    function updateAgreementStateSlot(address account, uint256 slotId, bytes32[] calldata slotData)
        external
        virtual
        override
    {
    ...
    function settleBalance(address account, int256 delta) external virtual override onlyAgreement {
