...
contract Superfluid is UUPSProxiable, ISuperfluid, BaseRelayRecipient {
     ...
     */
    // solhint-disable-next-line var-name-mixedcase
    uint256 public constant MAX_APP_CALLBACK_LEVEL = 1;
    ...
    function updateSuperTokenLogic(ISuperToken token, address newLogicOverride) external override onlyGovernance {
        address newLogic =
            newLogicOverride != address(0) ? newLogicOverride : address(_superTokenFactory.getSuperTokenLogic());
    ...
    function _enforceAppRegistrationPermissioning(string memory registrationKey, address deployer) internal view {
        ...
        if (
            // solhint-disable-next-line not-rely-on-time
            _gov.getConfigAsUint256(this, ISuperfluidToken(address(0)), configKey) < block.timestamp
        ) {
    ...
    function _callAppAction(address msgSender, ISuperApp app, uint256 value, bytes memory callData)
        internal
        cleanCtx
        isAppActive(app)
        ...
        for (uint256 i = 0; i < operations.length; ++i) {
            ...
            // for the contract to receive native tokens.
                ...
                (bool success, bytes memory returnData) =
                    SIMPLE_FORWARDER.forwardCall{value: address(this).balance}(operations[i].target, operations[i].data);
                if (!success) {
                    CallUtils.revertFromReturnedData(returnData);
                }
            } else if (operationType == BatchOperation.OPERATION_TYPE_ERC2771_FORWARD_CALL) {
                (bool success, bytes memory returnData) = _ERC2771_FORWARDER.forward2771Call{
                    value: address(this).balance
                }(operations[i].target, msgSender, operations[i].data);
                if (!success) {
                    CallUtils.revertFromReturnedData(returnData);
                }
            } else {
    ...
    function forwardBatchCall(Operation[] calldata operations) external payable override {
    ...
    function _callExternalWithReplacedCtx(address target, bytes memory callData, uint256 value, bytes memory ctx)
        private
