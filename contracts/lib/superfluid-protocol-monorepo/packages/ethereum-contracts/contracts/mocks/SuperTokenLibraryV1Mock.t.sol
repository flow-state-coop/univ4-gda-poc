...
contract SuperTokenLibraryCFAMock {
    ...
    function increaseFlowRateAllowanceWithPermissionsWithUserDataTest(
        ...
        token.increaseFlowRateAllowanceWithPermissions(flowOperator, permissionsToAdd, addedFlowRateAllowance, userData);
    }
...
contract SuperTokenLibraryGDAMock {
    ...
    function createPoolTest(ISuperToken token, address admin, PoolConfig memory config) external {
        token.createPool(admin, config);
    }

    function distributeTest(ISuperToken token, address from, ISuperfluidPool pool, uint256 requestedAmount) external {
