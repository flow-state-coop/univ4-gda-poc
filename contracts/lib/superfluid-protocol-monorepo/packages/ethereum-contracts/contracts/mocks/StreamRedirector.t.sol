...
contract StreamRedirector is SuperAppBase {
    ...
    function startStreamToSelf(address _originAccount, int96 _flowRate) public {
        token.createFlowFrom(_originAccount, address(this), _flowRate);
    }
    ...
    {
        newCtx = _ctx;
        newCtx = _superToken.createFlowWithCtx(receiver, _superToken.getNetFlowRate(address(this)), newCtx);
    }
    ...
    function afterAgreementTerminated(
        ...
        if (currentFlowRate > 0) {
            newCtx = _superToken.deleteFlowWithCtx(address(this), receiver, newCtx);
        }
    }
