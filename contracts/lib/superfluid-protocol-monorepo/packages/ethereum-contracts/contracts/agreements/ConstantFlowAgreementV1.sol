...
contract ConstantFlowAgreementV1 is AgreementBase, IConstantFlowAgreementV1 {
    ...
    function _getDepositRequiredForFlowRatePure(uint256 minimumDeposit, uint256 liquidationPeriod, int96 flowRate)
        internal
        pure
        ...
        returns (bool)
    {
        ...
        return SolvencyHelperLibrary.isPatricianPeriod(
            availableBalance, signedTotalCFADeposit, liquidationPeriod, patricianPeriod
        );
    }
    ...
    function getNetFlow(ISuperfluidToken token, address account) external view override returns (int96 flowRate) {
        (, FlowData memory state) = _getAccountFlowState(token, account);
    ...
    function _deleteFlow(
        ...
        if (
            currentContext.msgSender != flowVars.sender && currentContext.msgSender != flowVars.receiver
                && !hasPermissions
        ) {
            if (
        ...
        if (currentContext.msgSender == flowVars.sender) {
        ...
        } /* liquidations or flowOperator deleting a flow */ else {
            ...
            if (ISuperfluid(msg.sender).isApp(ISuperApp(flowVars.sender)) && availableBalance < 0) {
                newCtx = ISuperfluid(msg.sender).jailApp(
                    newCtx, ISuperApp(flowVars.sender), SuperAppDefinitions.APP_RULE_NO_CRITICAL_SENDER_ACCOUNT
                );
            }
    ...
    function createFlowByOperator(
...

        {
            ...
            (bytes32 flowOperatorId, uint8 permissions, int96 flowRateAllowance) =
            ...
            int96 updatedFlowRateAllowance =
                flowRateAllowance == type(int96).max ? flowRateAllowance : flowRateAllowance - flowRate;
        ...
        returns (bytes memory newCtx)
    {
        newCtx = updateFlowOperatorPermissions(
            token, flowOperator, FlowOperatorDefinitions.AUTHORIZE_FULL_CONTROL, type(int96).max, ctx
        );
    }
    ...
    function _changeFlowToNonApp(
        ...
        (depositDelta,, newFlowData) =
            _changeFlow(currentContext.timestamp, currentContext.appCreditToken, token, flowParams, oldFlowData);
    ...
    function _changeFlowToApp(
        ...
        if (appToCallback != address(0)) {
            AgreementLibrary.CallbackInputs memory cbStates = AgreementLibrary.createCallbackInputs(
                token, appToCallback, flowParams.flowId, abi.encode(flowParams.sender, flowParams.receiver)
            );
            ...
            } /* if (optype == FlowChangeType.DELETE_FLOW) */ else {
            ...
            // (closing streams)
            uint256 additionalAppCreditAmount =
                cbStates.appCreditGranted == 0 ? 0 : AgreementLibrary.max(DEFAULT_MINIMUM_DEPOSIT, minimumDeposit);
    ...
    function _changeFlow(
        ...
        FlowParams memory flowParams,
        FlowData memory oldFlowData
    ) private returns (int256 depositDelta, uint256 appCreditBase, FlowData memory newFlowData) {
        uint256 newDeposit;
        {
            ...
            depositDelta =
                appCreditBase.toInt256() - oldFlowData.deposit.toInt256() + oldFlowData.owedDeposit.toInt256();
            ...
            if (newDeposit < minimumDeposit && flowParams.flowRate > 0) {
                depositDelta =
                    minimumDeposit.toInt256() - oldFlowData.deposit.toInt256() + oldFlowData.owedDeposit.toInt256();
        ...
        }
        {
            ...
            vars.totalSenderFlowRate = _updateAccountFlowState(
                token, flowParams.sender, oldFlowData.flowRate - flowParams.flowRate, depositDelta, 0, currentTimestamp
            );
    ...
    function _encodeFlowData(FlowData memory flowData) internal pure returns (bytes32[] memory data) {
        ...
        data = new bytes32[](1);
        data[0] = bytes32(
            ((uint256(flowData.timestamp)) << 224) | ((uint256(uint96(flowData.flowRate)) << 128))
                | (uint256(flowData.deposit) >> 32 << 64) | (uint256(flowData.owedDeposit) >> 32)
        );
    }
        ...
        returns (bytes32[] memory data)
    {
        ...
        data = new bytes32[](1);
        data[0] =
            bytes32(uint256(flowOperatorData.permissions) << 128 | uint256(int256(flowOperatorData.flowRateAllowance)));
    }
