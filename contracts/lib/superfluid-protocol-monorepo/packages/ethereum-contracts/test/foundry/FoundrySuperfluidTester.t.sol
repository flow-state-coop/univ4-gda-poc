...
contract FoundrySuperfluidTester is Test {
    ...
    function _listAccounts() internal view returns (address[] memory accounts) {
        ...
        for (uint256 i = 0; i < OTHER_ACCOUNTS.values().length; ++i) {
    ...
    function _helperCreatePoolWithCustomERC20Metadata(
        ...
        vm.startPrank(_caller);
        localPool = SuperfluidPool(
            address(sf.gda.createPoolWithCustomERC20Metadata(_superToken, _poolAdmin, _poolConfig, _poolERC20Metadata))
        );
        vm.stopPrank();
    ...
    function _assertPoolCreation(
        ...
        // Assert ERC20 Metadata as expected
        {
            if (_useCustomERC20Metadata) {
                assertEq(
                    localPool.name(), _poolERC20Metadata.name, "_helperCreatePool: Pool ERC20 Metadata name mismatch"
                );
                assertEq(
                    localPool.symbol(),
                    _poolERC20Metadata.symbol,
                    "_helperCreatePool: Pool ERC20 Metadata symbol mismatch"
                );
                assertEq(
                    localPool.decimals(),
                    _poolERC20Metadata.decimals,
                    "_helperCreatePool: Pool ERC20 Metadata decimals mismatch"
                );
            } else {
    ...
    function _helperUpdateMemberUnits(
        ...
        if (!isConnected) {
            ...
            assertEq(balanceAfter, balanceBefore, "_helperUpdateMemberUnits: Pending balance changed");
        }
        ...
        internal
    {
        ...
        assertEq(sf.gda.isMemberConnected(pool_, caller_), false, "GDAv1.t D/C: Member not disconnected");
    ...
    function _helperDistributeViaGDA(
        ...
        // Assert Distributor RTB
        {
            ...
            uint256 amountReceivedInitial =
                sf.gda.isMemberConnected(pool_, from_) ? uint256(pool_.getUnits(from_)) * amountPerUnit : 0;
            assertEq(
                fromRTBAfter,
