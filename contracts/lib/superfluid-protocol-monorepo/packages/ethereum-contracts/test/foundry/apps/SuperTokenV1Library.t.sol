...
contract SuperTokenV1LibraryTest is FoundrySuperfluidTester {
    ...
    function testCreatePoolWithCustomERC20Metadata() external {
        ISuperfluidPool pool = superToken.createPoolWithCustomERC20Metadata(
            alice, PoolConfig(true, true), PoolERC20Metadata("My Token", "MTK", 6)
        );
    ...
    function testGetTotalAmountReceivedFromPool() external {
        ...
        assertEq(
            superToken.getTotalAmountReceivedFromPool(pool, bob), superToken.getTotalAmountReceivedByMember(pool, bob)
        );
    }
    ...
    function testGetGDAFlowInfo() external {
        ...
        (uint256 lastUpdated1, int96 flowRate1, uint256 deposit1) = superToken.getGDAFlowInfo(address(this), pool);
        assertEq(flowRate1, 0);
        assertEq(deposit1, 0);

        superToken.flowX(address(pool), DEFAULT_FLOWRATE);

        (uint256 lastUpdated2, int96 flowRate2, uint256 deposit2) = superToken.getGDAFlowInfo(address(this), pool);
    ...
    function testGetFlowInfoWithGDA() external {
        ...
        (
            uint256 lastUpdatedToPoolBefore,
            int96 flowRateToPoolBefore,
            uint256 depositToPoolBefore,
            uint256 owedDepositToPoolBefore
        ...
        (
            uint256 lastUpdatedToPoolAfter,
            int96 flowRateToPoolAfter,
            uint256 depositToPoolAfter,
            uint256 owedDepositToPoolAfter
        ) = superToken.getFlowInfo(address(this), address(pool));
    ...
    function testDistributeAndDistributeFlowAndReturnCorrectActualFlowrate(
        ...
        // distributeFlow from sender1 flowRate1
        {
            ...
            assertEq(
                actualFr, superToken.getFlowRate(address(sender1), address(pool)), "step 1 unexpected actual flowrate"
            );
            ...
            assertEq(
                actualAmount, balBefore - superToken.balanceOf(address(sender1)), "step 1 unexpected actual amount"
            );
        }
        ...
        // distributeFlow from sender2 flowRate2
        {
            ...
            assertEq(
                actualFr, superToken.getFlowRate(address(sender2), address(pool)), "step 2 unexpected actual flowrate"
            );
            ...
            assertEq(
                actualAmount, balBefore - superToken.balanceOf(address(sender2)), "step 2 unexpected actual amount"
            );
        }
        ...
        // distributeFlow from sender1 flowRate3
        {
            ...
            assertEq(
                actualFr, superToken.getFlowRate(address(sender1), address(pool)), "step 3 unexpected actual flowrate"
            );
            // distribute from sender1 amount3
            uint256 balBefore = superToken.balanceOf(address(sender1));
            uint256 actualAmount = sender1.distribute(superToken, pool, a3);
            assertEq(
                actualAmount, balBefore - superToken.balanceOf(address(sender1)), "step 3 unexpected actual amount"
            );
        }
    }
