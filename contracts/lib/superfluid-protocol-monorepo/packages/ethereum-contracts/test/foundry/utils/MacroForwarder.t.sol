...
contract GoodMacro is IUserDefinedMacro {
    ...
    function paramsCreateFlows(ISuperToken token, int96 flowRate, address[] calldata recipients)
        external
        pure
...
contract MacroForwarderTest is FoundrySuperfluidTester {
    ...
    function testStatefulMacro() external {
        ...
        m.setConfig(StatefulMacro.Config(sf.macroForwarder, superToken, 42, recipients, dan));
        vm.startPrank(admin);
    ...
    function testPostCheck() external {
        ...
        sf.macroForwarder.runMacro(
            m, abi.encode(superToken, alice, recipients, danBalanceBefore + (uint256(uint96(flowRate)) * 600))
        );
    }
    ...
    function testPaidCFAOps() external {
        ...
        sf.macroForwarder.runMacro{value: feeAmount}(m, m.encodeCreateFlow(superToken, bob, flowRate1));
        ...
        sf.macroForwarder.runMacro{value: feeAmount}(m, m.encodeUpdateFlow(superToken, bob, flowRate2));
        ...
        sf.macroForwarder.runMacro{value: feeAmount}(m, m.encodeDeleteFlow(superToken, alice, bob));
