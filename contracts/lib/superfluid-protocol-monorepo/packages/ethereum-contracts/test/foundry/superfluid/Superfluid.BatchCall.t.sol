...
contract SuperfluidBatchCallTest is FoundrySuperfluidTester {
    ...
    function testSimpleForwardCall() public {
        ...
        (bool success, bytes memory returnValue) =
            forwarder.forwardCall(address(testContract), abi.encodeCall(testContract.permissionlessFn, ()));
    ...
    function test2771ForwardCall() public {
        ...
        (bool success, bytes memory returnValue) =
            forwarder.forward2771Call(address(testContract), alice, abi.encodeCall(testContract.privilegedFn, ()));
        // decoded return value
        bool retVal = abi.decode(returnValue, (bool));
        ...
        (success,) =
            forwarder.forward2771Call(address(testContract), bob, abi.encodeCall(testContract.privilegedFn, ()));
    ...
    function testWithdrawLostNativeTokensFromSimpleForwarder() public {
        ...
        (bool success,) = forwarder.forwardCall{value: amount}(address(sf.host), new bytes(0x1));
    ...
    function testWithdrawLostNativeTokensFromERC2771Forwarder() public {
        ...
        // failing call which causes `amount` to get stuck in the forwarder contract
        (bool success,) = forwarder.forward2771Call{value: amount}(address(sf.host), address(this), new bytes(0x1));
