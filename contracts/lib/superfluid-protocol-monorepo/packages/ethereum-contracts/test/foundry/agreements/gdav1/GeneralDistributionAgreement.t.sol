...
contract GeneralDistributionAgreementV1IntegrationTest is FoundrySuperfluidTester {
    ...
    function testRevertDistributeFlowToNonPool(int96 requestedFlowRate) public {
        ...
        sf.host.callAgreement(
            sf.gda,
            abi.encodeCall(
                sf.gda.distributeFlow, (superToken, alice, ISuperfluidPool(bob), requestedFlowRate, new bytes(0))
            ),
            new bytes(0)
        );
        vm.stopPrank();
    ...
    function testRevertDistributeFromAnyAddressWhenNotAllowed(bool useForwarder) public {
        ...
        vm.startPrank(bob);
        sf.host.callAgreement(
            sf.gda, abi.encodeCall(sf.gda.distribute, (superToken, bob, pool, 1, new bytes(0))), new bytes(0)
        );
        vm.stopPrank();
    }
    ...
    function testRevertDistributeFlowFromAnyAddressWhenNotAllowed(bool useForwarder) public {
        ...
        vm.startPrank(bob);
        sf.host.callAgreement(
            sf.gda, abi.encodeCall(sf.gda.distributeFlow, (superToken, bob, pool, 1, new bytes(0))), new bytes(0)
        );
        vm.stopPrank();
    }
    ...
    function testRevertIfNotAdminUpdatesMemberUnitsViaGDA(bool useForwarder, PoolConfig memory config) public {
        ...
        sf.host.callAgreement(
            sf.gda, abi.encodeCall(sf.gda.updateMemberUnits, (pool, bob, 69, new bytes(0))), new bytes(0)
        );
        vm.stopPrank();
    }
    ...
    function testRevertIfUnitsTransferReceiverIsPool(address from, int96 unitsAmount, int128 transferAmount) public {
    ...
    function testBasicTransfer(
        FoundrySuperfluidTester._StackVars_UseBools memory useBools_,
        uint8 a,
        uint8 b, // One must use small sized data type to find equality cases
        uint128 unitsAmount,
