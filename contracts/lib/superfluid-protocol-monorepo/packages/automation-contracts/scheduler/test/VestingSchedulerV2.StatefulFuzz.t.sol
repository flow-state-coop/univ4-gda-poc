...
import {FlowOperatorDefinitions} from
...
contract VestingSchedulerV2StatefulFuzzTests is FoundrySuperfluidTester {
    ...
    function test_StatefulFuzz(Actions[10] calldata actions) external {
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            vm.assume(vestingScheduler.getVestingSchedule(address(superToken), sender, a.receiver).flowRate == 0);

            vm.assume(a.receiver != address(0) && sender != a.receiver);
            a.flowRate = int96(bound(a.flowRate, 1, 1000e18));
            a.startDate = uint32(bound(a.startDate, uint32(block.timestamp), uint32(block.timestamp + 3650 days)));
            a.endDate = uint32(
                bound(
                    a.endDate,
            ...
            if (a.cliffDate == 0) {
                ...
                a.cliffDate = uint32(bound(a.cliffDate, a.startDate, a.endDate - 1));
                vm.assume(a.endDate - a.cliffDate > vestingScheduler.MIN_VESTING_DURATION());
                vm.assume(
            ...
            if (t == TestActionType.TA_CREATE_SCHEDULE) {
                _test_createVestingSchedule(
                    sender, a.receiver, a.startDate, a.cliffDate, a.flowRate, a.cliffAmount, a.endDate
                );
            } else if (t == TestActionType.TA_UPDATE_SCHEDULE) {
                _test_updateVestingSchedule(
                    sender, a.receiver, a.startDate, a.cliffDate, a.flowRate, a.cliffAmount, a.endDate, a.newEndDate
                );
    ...
    function _test_createVestingSchedule(
        ...
        _createVestingSchedule(sender, receiver, startDate, cliffDate, flowRate, cliffAmount, endDate);

        //assert storage data
        VestingSchedulerV2.VestingSchedule memory schedule =
            vestingScheduler.getVestingSchedule(address(superToken), sender, receiver);

        uint32 cliffAndFlowDate = cliffDate == 0 ? startDate : cliffDate;
        assertTrue(schedule.cliffAndFlowDate == cliffAndFlowDate, "schedule.cliffAndFlowDate");
        ...
        assertTrue(schedule.claimValidityDate == 0, "schedule.claimValidityDate");
    }
    ...
    function _test_executeCliffAndFlow(
        ...
        bool success = vestingScheduler.executeCliffAndFlow(superToken, sender, receiver);
    ...
    function _test_updateVestingSchedule(
        ...
        vm.prank(sender);
        vestingScheduler.updateVestingSchedule(superToken, receiver, newEndDate, EMPTY_CTX);
    ...
    function _test_executeEndVesting(
        ...
        bool success = vestingScheduler.executeEndVesting(superToken, sender, receiver);
    ...
    function _createVestingSchedule(
        ...
        vestingScheduler.createVestingSchedule(
            superToken, receiver, startDate, cliffDate, flowRate, cliffAmount, endDate, 0, EMPTY_CTX
        );
    }
