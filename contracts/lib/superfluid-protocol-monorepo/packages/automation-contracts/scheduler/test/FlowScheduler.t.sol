...
contract FlowSchedulerTest is FoundrySuperfluidTester {
    ...
    function getHashID(address _superToken, address sender, address receiver) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(_superToken, sender, receiver));
    }
    ...
    function testCreateFlowScheduleWithZeroTimes() public {
        ...
        emit FlowScheduleCreated(
            superToken, alice, bob, 0, startMaxDelay, defaultFlowRate, defaultEndDate, defaultStartAmount, ""
        );
        vm.expectCall(
            address(flowScheduler),
            abi.encodeCall(
                flowScheduler.createFlowSchedule,
                (superToken, bob, 0, startMaxDelay, defaultFlowRate, defaultStartAmount, defaultEndDate, "", "")
            )
        );
        flowScheduler.createFlowSchedule(
            superToken, bob, 0, startMaxDelay, defaultFlowRate, defaultStartAmount, defaultEndDate, "", ""
    ...
    function testCannotCreateFlowScheduleWhenTimeWindowInvalid() public {
        ...
        flowScheduler.createFlowSchedule(
            superToken, bob, uint32(0), uint32(60), int96(1000), defaultStartAmount, uint32(0), "", ""
        );
    }
    ...
    function testExecuteCreateFlow() public {
        ...
        vm.warp(100);
        vm.expectCall(
            address(flowScheduler), abi.encodeCall(flowScheduler.executeCreateFlow, (superToken, alice, bob, ""))
        );
    ...
    function testExecute2xCreateFlow() public {
        ...
        vm.startPrank(admin);
        vm.expectEmit(true, true, true, true);
        emit CreateFlowExecuted(superToken, alice, bob, defaultStartDate, uint32(100), int96(1000), 0, "");
        vm.warp(100);
        vm.expectCall(
            address(flowScheduler), abi.encodeCall(flowScheduler.executeCreateFlow, (superToken, alice, bob, ""))
        );
