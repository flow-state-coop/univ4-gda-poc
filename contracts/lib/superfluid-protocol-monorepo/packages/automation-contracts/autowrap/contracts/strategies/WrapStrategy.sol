...
contract WrapStrategy is StrategyBase {
    ...
    function wrap(address user, ISuperToken superToken, uint256 superTokenAmount) external override {
        ...
        (uint256 underlyingAmount, uint256 adjustedAmount) =
            _toUnderlyingAmount(superTokenAmount, underlyingToken.decimals());

        // Transfer the underlying tokens from the user
        underlyingToken.safeTransferFrom(user, address(this), underlyingAmount);
        ...
        if (underlyingToken.allowance(address(this), address(superToken)) <= underlyingAmount) {
            underlyingToken.safeIncreaseAllowance(address(superToken), type(uint256).max);
        }
    ...
    function isSupportedSuperToken(ISuperToken superToken) public view override returns (bool) {
