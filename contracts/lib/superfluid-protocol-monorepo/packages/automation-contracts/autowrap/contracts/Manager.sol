...
contract Manager is IManager, Ownable {
    ...
    function createWrapSchedule(
        ...
        if (wrapSchedule[index].user != msg.sender) {
            if (superToken == address(0) || strategy == address(0) || liquidityToken == address(0)) {
                revert ZeroAddress();
            }
            ...
            if (!IStrategy(strategy).isSupportedSuperToken(ISuperToken(superToken))) {
                revert UnsupportedSuperToken(address(superToken));
            }

            WrapSchedule memory wrap = WrapSchedule( // create new Wrap or update wrap
            msg.sender, ISuperToken(superToken), IStrategy(strategy), liquidityToken, expiry, lowerLimit, upperLimit);
        ...
        emit WrapScheduleCreated(
            index, msg.sender, superToken, strategy, liquidityToken, expiry, lowerLimit, upperLimit
        );
    }

    /// @dev IManager.executeWrap implementation.
    function executeWrap(address user, address superToken, address liquidityToken) external override {
        executeWrapByIndex(getWrapScheduleIndex(user, superToken, liquidityToken));
    }

    /// @dev IManager.deleteWrapSchedule implementation.
    function deleteWrapSchedule(address user, address superToken, address liquidityToken) external override {
    ...
    function addApprovedStrategy(address strategy) external override onlyOwner {
    ...
    function setLimits(uint64 lowerLimit, uint64 upperLimit) external onlyOwner {
    ...
    {
        return getWrapScheduleByIndex(getWrapScheduleIndex(user, superToken, liquidityToken));
    }

    /// @dev IManager.checkWrap implementation.
    function checkWrap(address user, address superToken, address liquidityToken)
        external
        view
        override
        returns (uint256)
    {
        return checkWrapByIndex(getWrapScheduleIndex(user, superToken, liquidityToken));
    }
    ...
    function deleteWrapScheduleByIndex(bytes32 index) public {
        ...
        emit WrapScheduleDeleted(
            index, wrap.user, address(wrap.superToken), address(wrap.strategy), wrap.liquidityToken
        );
    ...
    function getWrapScheduleByIndex(bytes32 index) public view returns (WrapSchedule memory) {
        return wrapSchedule[index];
    }

    /// @dev IManager.checkWrapByIndex implementation.
    function checkWrapByIndex(bytes32 index) public view returns (uint256 amount) {
        WrapSchedule storage wrap = wrapSchedule[index];
        ...
        if (
            wrap.user == address(0) // Task exists and has a valid user
                ...
                ) == 0 || IERC20Mod(wrap.liquidityToken).balanceOf(wrap.user) == 0 // check user balance
