...
contract WrapStrategyTests is FoundrySuperfluidTester {
    ...
    event Wrap(address indexed user, address indexed superToken, uint256 superTokenAmount);
    ...
    function testSupportedSuperToken() public {
        assertTrue(wrapStrategy.isSupportedSuperToken(superToken), "SuperToken should be supported");
    }

    function testCannotNonSupportedSuperToken() public {
        assertTrue(!wrapStrategy.isSupportedSuperToken(nativeSuperToken), "Native SuperToken shouldn't be supported");
    }
