...
contract ManagerTests is FoundrySuperfluidTester {
    ...
    function getWrapIndex(address user, address superToken_, address liquidityToken) public pure returns (bytes32) {
    ...
    function test_RevertWhen_DeploymentWrongLimits() public {
        vm.expectRevert(abi.encodeWithSelector(IManager.WrongLimits.selector, 2, 1));
    ...
    function testCannotCreateWrapWrongData() public {
        ...
        vm.expectRevert(IManager.ZeroAddress.selector);
        manager.createWrapSchedule(address(0), address(wrapStrategy), address(token), EXPIRY, MIN_LOWER, MIN_UPPER);

        // revert with _strategy = 0
        vm.expectRevert(IManager.ZeroAddress.selector);
        manager.createWrapSchedule(address(superToken), address(0), address(token), EXPIRY, MIN_LOWER, MIN_UPPER);

        // revert with liquidityToken = 0
        vm.expectRevert(IManager.ZeroAddress.selector);
        manager.createWrapSchedule(address(superToken), address(wrapStrategy), address(0), EXPIRY, MIN_LOWER, MIN_UPPER);

        // revert with expiry < now
        vm.warp(2);
        vm.expectRevert(abi.encodeWithSelector(IManager.InvalidExpirationTime.selector, 1, block.timestamp));
        manager.createWrapSchedule(address(superToken), address(wrapStrategy), address(token), 1, MIN_LOWER, MIN_UPPER);

        // revert with MIN_LOWER < minLower
        vm.expectRevert(abi.encodeWithSelector(IManager.InsufficientLimits.selector, 1, MIN_LOWER));
        manager.createWrapSchedule(address(superToken), address(wrapStrategy), address(token), EXPIRY, 1, MIN_UPPER);

        // revert with MIN_UPPER < minUpper
        vm.expectRevert(abi.encodeWithSelector(IManager.InsufficientLimits.selector, 1, MIN_UPPER));
        manager.createWrapSchedule(address(superToken), address(wrapStrategy), address(token), EXPIRY, MIN_LOWER, 1);
        ...
        manager.createWrapSchedule(
            address(superToken), address(wrapStrategy), address(token), EXPIRY, MIN_LOWER, MIN_UPPER
        );
        vm.startPrank(admin);
        manager.addApprovedStrategy(address(wrapStrategy));
        vm.stopPrank();
        ...
        manager.createWrapSchedule(
            address(nativeSuperToken), address(wrapStrategy), address(token), EXPIRY, MIN_LOWER, MIN_UPPER
        );
    }
