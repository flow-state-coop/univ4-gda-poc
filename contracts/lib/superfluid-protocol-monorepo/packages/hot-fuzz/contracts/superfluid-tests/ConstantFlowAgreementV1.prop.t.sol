...
contract CFAProperties {
    ...
    function testMaximumFlowRateAllowedForDeposit(uint32 liquidationPeriod, uint96 depositAllowed) public view {
        ...
        require(
            uint256(liquidationPeriod) * uint256(depositAllowed) <= uint256(uint96(type(int96).max)), "FOUNDRY::ASSUME"
        );
    ...
    function testMinimumDeposit(uint64 minimumDeposit, uint32 liquidationPeriod, int96 flowRate) public view {
        ...
        require(
            uint256(liquidationPeriod) * uint256(uint96(flowRate)) <= uint256(uint96(type(int96).max)),
            "FOUNDRY::ASSUME"
        );
